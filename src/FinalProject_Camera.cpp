...
int main(int argc, const char *argv[])
{
    ...
    FILE* ttc_camera = fopen("/home/brolin/Desktop/time_test.txt", "a");
    FILE* ttc_lidar = fopen("/home/brolin/Desktop/lidar_test.txt", "a");
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex+=imgStepWidth)
    {
        ...
        checkDetectorDescriptorCompatibility(detectorType, descriptorType);
        ...
        if(!info_written)
        {
            fprintf(ttc_camera, "%s-%s\t", detectorType.c_str(), descriptorType.c_str());
            fprintf(ttc_lidar, "%s\t", "TTC LIDAR");
            info_written = true;
        }
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if( currBB->lidarPoints.size()>0 && prevBB->lidarPoints.size()>0 ) // only compute TTC if we have Lidar points
                {
                    ...
                    if (bVis)
                    {
                        ...
                        cv::waitKey(10); //wait for 0.5 seconds before next loop. Time in milliseconds

                        fprintf(ttc_camera, "%lf\t", ttcCamera);
                        fprintf(ttc_lidar, "%lf\t", ttcLidar);
                    }
                    bVis = false;
